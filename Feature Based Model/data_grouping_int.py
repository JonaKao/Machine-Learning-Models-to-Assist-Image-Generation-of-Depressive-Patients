...
"""
Aggregate OpenFace AU features by PHQ-8 severity groups (conditioned on AU presence).
...
2) For each participant:
   ...
   - For each AU pair (AUxx_r, AUxx_c) present:
       * Compute mean(AUxx_r) over rows where AUxx_c == 1
       * If no rows satisfy AUxx_c == 1, record NaN and log a note
   - Ignore all other columns (pose, gaze, *_c, etc.)
3) Save per-patient AU means to per_patient_means.csv
...
def compute_patient_means_au_conditioned(csv_path: Path, verbose: bool = True) -> pd.Series:
    """
    Compute per-AU means of *_r columns conditioned on the corresponding *_c == 1.
    Only returns AU_r columns (no *_c, no pose/gaze).
    """
    if verbose:
        print(f"  - Reading {csv_path}")
    df = pd.read_csv(csv_path)

    # Drop excluded columns if present (not strictly needed for AU selection)
    ...
    # Identify AU pairs: columns ending with '_r' that have a matching '_c'
    au_r_cols = [c for c in df.columns if c.endswith('_r')]
    pairs = []
    for aur in au_r_cols:
        auc = aur[:-2] + '_c'  # replace suffix
        if auc in df.columns:
            # Ensure both are numeric or coercible to numeric
            # (OpenFace usually outputs numeric, but we guard anyway)
            pairs.append((aur, auc))

    if not pairs:
        raise ValueError(
            "No AU pairs (AUxx_r with matching AUxx_c) found in file. "
            "Check column names or file content."
        )

    # Convert to numeric to be safe (errors='coerce' -> NaN if any stray text)
    for aur, auc in pairs:
        df[aur] = pd.to_numeric(df[aur], errors='coerce')
        df[auc] = pd.to_numeric(df[auc], errors='coerce')

    # Compute conditioned means
    means = {}
    for aur, auc in pairs:
        active = df[df[auc] == 1]
        if active.empty:
            if verbose:
                print(f"    > NOTE: No frames with {auc}==1; mean({aur}|{auc}=1) set to NaN")
            means[aur] = float('nan')
        else:
            means[aur] = active[aur].mean()

    return pd.Series(means)


def main():
    parser = argparse.ArgumentParser(description="Aggregate OpenFace AU_r means conditioned on AU_c==1 by PHQ-8 groups.")
    ...
    for idx, (pid, phq) in enumerate(meta[["Participant_ID", "PHQ_Score"]].itertuples(index=False, name=None), start=1):
        ...
        patient_file = patient_folder / "features" / file_name  # includes 'features' subfolder
        ...
        if not patient_folder.exists():
            ...
            means = compute_patient_means_au_conditioned(patient_file, verbose=verbose)
            row = {
                "Participant_ID": int(pid),
                "PHQ_Score": float(phq),
                "PHQ_Category": category,
            }
            # Only AU_r conditioned means are included
    ...
    if verbose:
        print(f"> Saving per-patient AU_r|AU_c==1 means to: {per_patient_csv}")
    per_patient_df.to_csv(per_patient_csv, index=False)

    if verbose:
        print("> Computing group-wise means (NaNs skipped)...")
    # Feature columns are all AU_r means we computed (exclude id/label columns)
    ...
    if not feature_cols:
        raise ValueError("No AU_r feature columns found to aggregate in per_patient_means.csv.")

    grouped = (
        per_patient_df
        .groupby("PHQ_Category", sort=False)[feature_cols]
        .mean(numeric_only=True)  # skipna=True by default
    ...
    if verbose:
        print("\nAll done! ✅")
        print(f"- Per-patient AU_r|AU_c==1 means: {per_patient_csv.resolve()}")
        print(f"- Group-wise AU_r|AU_c==1 means: {group_means_csv.resolve()}")
